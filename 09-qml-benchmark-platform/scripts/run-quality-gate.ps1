...
Write-Host "[1/7] Validating versioned Sprint 3 release assets"
Invoke-Checked "Contracts, boundaries and repository artifacts" {
    docker run --rm --volume "${PWD}:/workspace" --workdir /workspace `
        python:3.12.11-slim-bookworm sh -c `
        "pip install --quiet jsonschema==4.25.1 && python scripts/validate-release.py && python scripts/validate-azure-release.py && python scripts/validate-repository.py"
}
...
$PreviousApiHostPort = $env:API_HOST_PORT
$PreviousDashboardHostPort = $env:DASHBOARD_HOST_PORT
$env:API_HOST_PORT = "0"
$env:DASHBOARD_HOST_PORT = "0"
try {
    ...
    Invoke-Checked "Repository content recheck" {
        docker run --rm --volume "${PWD}:/workspace" --workdir /workspace `
            qml-benchmark-quality-gate:s3 sh -c `
            "python scripts/validate-release.py && python scripts/validate-azure-release.py && python scripts/validate-repository.py"
    }

    Write-Host "OK - Project 09 Sprint 3 quality gate passed"
}
finally {
    docker compose down --remove-orphans
    if ($null -eq $PreviousApiHostPort) {
        Remove-Item Env:API_HOST_PORT -ErrorAction SilentlyContinue
    }
    else {
        $env:API_HOST_PORT = $PreviousApiHostPort
    }
    if ($null -eq $PreviousDashboardHostPort) {
        Remove-Item Env:DASHBOARD_HOST_PORT -ErrorAction SilentlyContinue
    }
    else {
        $env:DASHBOARD_HOST_PORT = $PreviousDashboardHostPort
    }
}
